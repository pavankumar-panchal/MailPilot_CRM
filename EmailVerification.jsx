...
const EmailVerification = () => {
  ...
  const fetchListEmails = async (listId) => {
    try {
      const res = await fetch(
        `http://localhost/Verify_email/backend/routes/api.php/api/results?csv_list_id=${listId}`
      );
      const data = await res.json();
        ...
        >
          ...
          >
            ...
            <h3 className="text-xl font-bold mb-4 text-gray-900">
              Select Lists to Export ({exportType.charAt(0).toUpperCase() + exportType.slice(1)} Emails)
            </h3>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">List Name</th>
                    <th className="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Status</th>
                    <th className="px-4 py-2 text-left text-xs font-semibold text-gray-700 uppercase">Emails</th>
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {lists.map((list) => (
                    <tr key={list.id}>
                      ...
                      <td className="px-4 py-2 text-gray-900">{list.list_name}</td>
                      <td className="px-4 py-2 text-gray-900">{list.status}</td>
                      <td className="px-4 py-2 text-gray-900">{list.total_emails}</td>
      ...
      <div className="bg-white rounded-xl shadow p-6">
                                    ...
                                    >
                                      ...
                                      <td className="px-3 py-1 text-xs text-gray-600">
                                        {email.validation_response?.length > 40
                                          ? email.validation_response.slice(
                                              0,
                                              40
                                            ) + "..."
        ...
        {lists.length > 0 && (
          <div className="flex flex-col px-1 items-center mt-6">
            ...
            <div className="flex items-center gap-2 justify-center bg-gray-50 py-3 rounded-lg border border-gray-200">
              <button
                onClick={() =>
