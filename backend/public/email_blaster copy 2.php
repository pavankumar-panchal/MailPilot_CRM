...
function processEmailBatch($db, $campaign_id, $total_email_count)
{
    ...
    foreach ($emails as $email) {
        ...
        try {
            ...
            // Success
            recordDelivery($db, $smtp['smtp_account_id'], $emailId, $campaign_id, $to, 'success', null, $smtp['received_email']);
            ...
            // Failure
            recordDelivery($db, $smtp['smtp_account_id'], $emailId, $campaign_id, $to, 'failed', $e->getMessage(), $smtp['received_email']);
...
function recordDelivery($db, $smtpId, $emailId, $campaignId, $to_email, $status, $error = null, $reply_to = null)
{
    $db->query("
        INSERT INTO mail_blaster 
        (campaign_id, to_mail, reply_to, smtpid, delivery_date, delivery_time, status, error_message, attempt_count)
        VALUES (
            $campaignId, 
            '" . $db->real_escape_string($to_email) . "', 
            " . ($reply_to ? "'" . $db->real_escape_string($reply_to) . "'" : "NULL") . ",
            $smtpId, 
            CURDATE(), 
            CURTIME(), 
