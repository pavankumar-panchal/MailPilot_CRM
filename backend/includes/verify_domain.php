...
if ($remainingDomains == 0) {
    consoleOutput("✅ All domains verified. Starting SMTP verification...");
    exec('php backend/includes/verify_smtp.php > /dev/null 2>&1 &');
    exit(0);
} else {
    consoleOutput("⏳ Still $remainingDomains domains left. Skipping SMTP for now.");
    exec('php backend/includes/verify_domain.php > /dev/null 2>&1 &');
